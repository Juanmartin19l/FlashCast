...
def mostrar_alerta(mensaje):
    ...
    # Deshabilitar cierre, minimizar y mover
    root.overrideredirect(True)  # Elimina los botones de ventana
    ...
    # Título parpadeante
    titulo = tk.Label(root, text="⚠️ MENSAJE URGENTE ⚠️", 
                     font=fuente, fg="yellow", bg="red")
    titulo.pack(pady=50)
    ...
    label = tk.Label(root, text=mensaje, font=fuente_mensaje, 
                    fg="white", bg="red", wraplength=root.winfo_screenwidth()-100)
    label.pack(expand=True, pady=20)
    
    # Instrucciones
    instruccion = tk.Label(root, text='Escribe "confirmar" para cerrar:', 
                          font=font.Font(size=24), fg="white", bg="red")
    ...
    # Mensaje de error
    error_label = tk.Label(root, text="", font=font.Font(size=18), 
                          fg="yellow", bg="red")
    ...
    boton = tk.Button(root, text="ACEPTAR", command=verificar_confirmacion, 
                     font=font.Font(size=24, weight="bold"), bg="yellow", fg="red", 
                     padx=40, pady=20)
    boton.pack(pady=20)
